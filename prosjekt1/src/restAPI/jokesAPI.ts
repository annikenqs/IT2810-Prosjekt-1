...
const fetchJoke = async (type: string): Promise<JokeResponse> => {
	const response = await fetch(
		`https://v2.jokeapi.dev/${type}?blacklistFlags=racist,sexist&idRange=0-318`,
	);
	if (!response.ok) {
...
const fetchJokeById = async (id: number): Promise<JokeResponse> => {
	...
	const response = await fetch(
		`https://v2.jokeapi.dev/joke/Programming,Spooky,Christmas?blacklistFlags=racist,sexist&idRange=${id}`,
	);
	if (!response.ok) {
		throw new Error("Failed to fetch joke");
	}
...
const fetchJokesByCategory = async (
	category: string,
	batchSize: number,
	rangeStart: number,
	rangeEnd: number,
